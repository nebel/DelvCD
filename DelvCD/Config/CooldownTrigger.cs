...
namespace DelvCD.Config
{
    public class CooldownTrigger : TriggerOptions
    {
        ...
        public override bool IsTriggered(bool preview)
        {
            ...
            if (RangeCheck)
            {
                inRange = helper.GetActionInRange(actionId, Singletons.ObjectTable.LocalPlayer, Utils.FindTarget());
            }

            if (LosCheck)
            {
                inLos = helper.IsTargetInLos(Singletons.ObjectTable.LocalPlayer, Utils.FindTarget(), actionId);
            }
